...
interface MarcasListProps {
    data: Marca[];
    columns: Column<Marca>[];
    onRowClick: (item: Marca) => void;
    onNew: () => void;
    loading?: boolean;
}

export const MarcasList: React.FC<MarcasListProps> = ({
    data,
    columns,
    onRowClick,
    onNew,
        ...
        <div className="flex flex-col h-full w-full relative">
            {loading && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/50 backdrop-blur-[2px] rounded-2xl transition-all duration-300">
                    ...
                    <span className="text-xs font-medium text-slate-500" aria-live="polite">Actualizando marcas…</span>
                </div>
            )}
