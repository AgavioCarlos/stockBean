...
interface ProveedoresListProps {
    data: IProveedor[];
    columns: Column<IProveedor>[];
    onRowClick: (item: IProveedor) => void;
    onNew: () => void;
    loading?: boolean;
}
        ...
        <div className="flex flex-col h-full w-full relative">
            {loading && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/50 backdrop-blur-[2px] rounded-2xl transition-all duration-300">
                    ...
                    <span className="text-xs font-medium text-slate-500" aria-live="polite">Actualizando proveedores…</span>
                </div>
            )}
