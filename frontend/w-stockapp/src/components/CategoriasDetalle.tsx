...
export default function CategoriasDetalle({
                ...
                <div className="flex items-center justify-between gap-4 px-6 py-3 border-b border-gray-100 shrink-0">
                    <div>
                        <h3 className="text-lg font-semibold text-gray-800">
                            {categoriaSeleccionada ? "Detalle de Categoría" : "Nueva Categoría"}
                        </h3>
                    </div>

                    <div className="flex items-center gap-3">
                        {!isEditing && categoriaSeleccionada && (
                            <button
                                type="button"
                                    ...
                                    <button
                                        ...
                                        onClick={async () => {
                                            ...
                                            const res = await Swal.fire({
                                                ...
                                                text: `¿Deseas desactivar "${categoriaSeleccionada.nombre}"? Esto marcará la categoría como inactiva.`,
                                                icon: 'warning',
                                            ...
                                            if (res.isConfirmed && typeof onDelete === 'function') {
                                                onDelete(categoriaSeleccionada.idCategoria as number, false);
                                            }
                                        }}
                                    ...
                                    <button
                                        ...
                                        onClick={async () => {
                                            ...
                                            const res = await Swal.fire({
                                                ...
                                                text: `¿Deseas activar "${categoriaSeleccionada.nombre}"? Esto marcará la categoría como activa.`,
                                                icon: 'question',
                                                showCancelButton: true,
                                                confirmButtonColor: '#d33',
                                                cancelButtonColor: '#d33',
                                            ...
                                            if (res.isConfirmed && typeof onDelete === 'function') {
                                                onDelete(categoriaSeleccionada.idCategoria as number, true);
                                            }
                                        }}
                                        className="px-4 py-1.5 text-green-600 text-sm font-medium rounded-md border border-green-600 bg-white hover:bg-green-600 hover:text-white transition-colors flex items-center gap-2"
                                    >
